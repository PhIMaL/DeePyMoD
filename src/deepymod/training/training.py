    ...
    for iteration in torch.arange(0, max_iterations): # iterate over epochs
        ...
        for batch_idx, train_sample in enumerate(train_dataloader):
            ...
            batch_losses[2, :, batch_idx] = torch.stack(
                [
                    torch.mean((dt - theta @ coeff_vector) ** 2) # equation (7) of the original paper, used to be called reg_loss in DeePyMoD_torch
        ...
        if iteration % write_iterations == 0:
            ...
            converged = convergence(iteration, l1_norm) # Check if change is smaller than delta and if we've exceeded patience
            if converged:
                break
    logger.close(model)
